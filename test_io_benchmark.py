...
def benchmark_io_methods():
    ...
    # === 方法2: dtype指定pandas ===
    start_time = time.perf_counter()
    dtype_map = {'Open': np.float32, 'High': np.float32, 'Low': np.float32, 'Close': np.float32, 'Volume': np.float32}
    for file_path in test_files:
        try:
    ...
    for file_path in test_files:
        try:
            ...
            if available_cols:
                df = pd.read_csv(file_path, usecols=available_cols, dtype=dtype_map, low_memory=False)
        except Exception:
            pass
    ...
    for file_path in test_files:
        try:
            ...
            data = np.genfromtxt(file_path, delimiter=',', skip_header=1, usecols=(1,2,3,4,5), dtype=np.float32, invalid_raise=False)
        except Exception:
            pass
