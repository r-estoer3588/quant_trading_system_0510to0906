...
class IOOptimizationBenchmark:
    ...
    def _get_file_sample(self, cache_dir: Path, 
                        sample_size: Optional[int] = None) -> List[Tuple[Path, float]]:
        """ファイルサンプル取得（パス, サイズMB）"""
    ...
    def _method_threaded_pandas(self, files: List[Tuple[Path, float]]) -> IOBenchmarkResult:
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = [executor.submit(load_single_file, file_info) 
    ...
    def run_comprehensive_benchmark(self, 
                                  profile: str = "rolling",
                                  sample_size: int = 20) -> List[IOBenchmarkResult]:
        """包括的I/Oベンチマーク実行"""
        ...
        for method in methods:
            ...
            try:
                ...
                logger.info(f"{result.method_name}: {result.wall_clock_seconds:.3f}s, "
    ...
    def generate_benchmark_report(self, results: List[IOBenchmarkResult]) -> Dict:
        ...
        # スループット比較
        baseline_throughput = next((r.throughput_mb_per_sec for r in results 
                                  if "Sequential" in r.method_name), 0)
        ...
        if lowest_memory.memory_peak_mb < best_throughput.memory_peak_mb * 0.8:
            recommendations.append(
                f"{lowest_memory.method_name}: メモリ効率優秀 ({lowest_memory.memory_peak_mb:.1f}MB) - "
                "大規模処理時推奨"
            )
...
def main():
    ...
    parser.add_argument("--profile", choices=["base", "rolling", "full_backup"],
                       default="rolling", help="ベンチマーク対象プロファイル")
    ...
    print(f"最高スループット: {perf['best_throughput']['method']} "
          f"({perf['best_throughput']['value']:.2f} MB/s)")
    print(f"最速実行時間: {perf['fastest_wall_time']['method']} "
          f"({perf['fastest_wall_time']['value']:.3f}秒)")
    print(f"最小メモリ: {perf['lowest_memory']['method']} "
