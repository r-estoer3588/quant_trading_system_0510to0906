# ============================================================================
# 🧠 Context Note
# このファイルは precomputed 指標へのアクセスヘルパー。大文字/小文字・別名の揺れを統一的に解決
#
# 前提条件：
#   - core/systemX.py は全て indicator_access.py 経由で指標にアクセス（直接アクセス禁止）
#   - 別名マッピングは確認された揺れのみ対応。新規別名追加は慎重に
#   - キャッシュから precomputed 指標ロード済みのデータを対象
#
# ロジック単位：
#   get_first()    → 複数の候補キーから最初に見つかった値を返す
#   normalize_key() → 大文字/小文字揺れを解決
#
# Copilot へ：
#   → 別名マッピング追加時は必ず背景・理由を残す
#   → 新指標追加時は最初にここで別名登録（複数 core に対応させるため）
#   → indicator_access を経由しないアクセスは絶対禁止
# ============================================================================
