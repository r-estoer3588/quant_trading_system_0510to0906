            ...
            if cached is not None and not cached.empty:
                ...
                try:
                    ...
                    if "Date" in cached_local.columns:
                        cached_dates = pd.to_datetime(cached_local["Date"], errors="coerce").dt.normalize()
                    else:
                        cached_dates = pd.to_datetime(cached_local.index, errors="coerce").normalize()
                    ...
                    use_cached_only = (
                        pd.notna(last) and pd.notna(src_latest) and src_latest <= last and len(cached_local) == len(src)
                    )
                    if use_cached_only:
                        ind_df = cached_local
