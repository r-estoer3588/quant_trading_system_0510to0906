...
def display_adx_ranking(
    ...
    if not candidates_by_date:
        st.warning(tr("ADX7ランキングチE�Eタがありません"))
        return
    ...
    # Streamlit の progress/write は DeltaGenerator を返すため、型の整合性を保つコールバックでラチE�E
    ...
    progress.empty()
    log_area.write(tr("ADX7ランキング完亁E))
    ...
    title = tr(
        "{display_name} ADX7 ランキング�E�直近{years}年 / 上位{top_n}銘柄�E�E,
...
def run_tab(ui_manager: UIManager | None = None) -> None:
    st.header(
        tr(
            "{display_name} バックチE��ト（ロング・ミ�Eンリバ�Eジョン�E�ADXフィルター�E�E,
            display_name=DISPLAY_NAME,
        )
    )
    ui_base: UIManager = (
