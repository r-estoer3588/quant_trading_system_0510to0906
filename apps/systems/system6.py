...
def display_return6d_ranking(
    ...
    if not candidates_by_date:
        st.warning(tr("return_6dランキングチE�Eタがありません"))
        return
    ...
    progress.empty()
    log_area.write(tr("return_6dランキング完亁E))
    ...
    title = tr(
        "{display_name} return_6d ランキング�E�直近{years}年 / 上位{top_n}銘柄�E�E,
...
def run_tab(ui_manager: UIManager | None = None) -> None:
    st.header(
        tr(
            "{display_name} バックチE��ト！Eeturn_6d ランキング�E�E,
            display_name=DISPLAY_NAME,
        )
    )
    ui_base: UIManager = (
