...
def display_drop3d_ranking(
    ...
    if not candidates_by_date:
        st.warning(tr("3日下落玁E��ンキングが空でぁE))
        return
    ...
    # st.progress の戻り値は DeltaGenerator なので、None を返すコールバックにラチE�Eする
    ...
    progress.empty()
    log_area.write(tr("3日下落玁E��ンキング完亁E))
    ...
    title = tr(
        "{display_name} 日別 3日下落玁Eランキング�E�直近{years}年 / 上位{top_n}銘柄�E�E,
...
def run_tab(ui_manager: UIManager | None = None) -> None:
    st.header(
        tr(
            "{display_name} バックチE��ト（ロング・ミ�Eンリバ�Eジョン�E�急落の反発狙い�E�E,
            display_name=DISPLAY_NAME,
        )
    )
    ui_base: UIManager = (
