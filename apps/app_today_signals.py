...
class RunCallbacks:
    ...
    def register_with_module(self) -> None:
        try:
            ...
            # Bulk exit callback: accept a dict of system->count for atomic updates
            try:
    ...
    with st.spinner("実行中... (経過時間表示あり)"):
        ...
        if run_config.scan_missing_only:
            ...
            result = compute_today_signals(
                ...
                exit_analysis_paper_mode=bool(paper_mode),
            )
...
def analyze_exit_candidates(paper_mode: bool) -> ExitAnalysisResult:
    ...
    try:
        from common.exit_analysis import analyze_exit_candidates as _analyze

        exits_today_df, planned_df, exit_counts, error = _analyze(
            paper_mode=paper_mode, client=None, skip_external=False
        )
        return ExitAnalysisResult(exits_today=exits_today_df, planned=planned_df, exit_counts=exit_counts, error=error)
