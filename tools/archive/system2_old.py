...
def prepare_data_vectorized_system2(
    ...
    **kwargs,
    ...
    if raw_data_dict:
        ...
        if log_callback:
            log_callback(f"System2 fast-path: processed {len(result_dict)} symbols, skipped {skipped_count}")
        return result_dict
    ...
    if log_callback:
        log_callback(f"System2 preparation complete: {len(result_dict)} symbols processed, {skipped_count} skipped")
...
def system2_backtest_vectorized(
    ...
    **kwargs,
    ...
    for i, current_date in enumerate(trading_dates):
        ...
        # Select candidates
        candidates = select_candidates_system2(data_dict, date_str, top_n=top_n, log_callback=log_callback)
    ...
    if log_callback:
        log_callback(f"System2 backtest complete: {total_trades} trades, {summary['total_return']:.4f} total return")
