...
def display_return6d_ranking(
    ...
    title = tr(
        "System6 Return6D ランキング（直近{years}年 / 上位{top_n}銘柄）", years=years, top_n=top_n
    )
...
def run_tab(ui_manager: UIManager | None = None) -> None:
    ...
    _rb = cast(
        ...
        run_backtest_app(strategy, system_name="System6", limit_symbols=100, ui_manager=ui),
    )
            ...
            for n in notifiers:
                try:
                    ...
                    if hasattr(n, "send_backtest_ex"):
                        n.send_backtest_ex(
                            "system6", period, stats, ranking, image_url=_img_url, mention=_mention
                        )
                    else:
