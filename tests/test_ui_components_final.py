...
class TestRemainingFunctionsCoverage:
        ...
        if hasattr(ui_components, "run_backtest_with_logging"):
            ...
            try:
                ui_components.run_backtest_with_logging(
                    prepared_dict, {}, mock_strategy, 10000.0, "Test"
                )
            except Exception:
    ...
    def test_clean_date_column_additional_formats(self):
        ...
        test_dataframes = [
            # ISO format
            pd.DataFrame({"Date": ["2023-01-01T00:00:00", "2023-01-02T00:00:00"], "Value": [1, 2]}),
            # Already datetime
            pd.DataFrame({"Date": pd.date_range("2023-01-01", periods=5), "Value": range(5)}),
            # Mixed valid/invalid
            pd.DataFrame({"Date": ["2023-01-01", "invalid", "2023-01-03"], "Value": [1, 2, 3]}),
        ]
    ...
    def test_fetch_data_edge_cases(self, mock_get_cached):
        ...
        scenarios = [
            ...
            (
                ...
                pd.DataFrame(
                    {"symbol": [f"SYM{i}" for i in range(200)], "close": np.random.rand(200) * 100}
                ),
            ),
            # Single symbol
            (["SINGLE"], pd.DataFrame({"symbol": ["SINGLE"], "close": [100.0]})),
            # Empty result
            (["EMPTY"], pd.DataFrame()),
        ]
