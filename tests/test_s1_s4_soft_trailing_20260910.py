...
def test_canonical_trailing_thresholds_are_hwm_based():
    assert soft.trailing_threshold(side="long", hwm=120.0, trail_pct=0.25) == pytest.approx(90.0)
    assert soft.trailing_threshold(side="long", hwm=120.0, trail_pct=0.20) == pytest.approx(96.0)


def test_hwm_ratchet_never_moves_down():
    assert soft.update_hwm(side="long", previous=120.0, entry=100.0, current=110.0) == 120.0
    assert soft.update_hwm(side="long", previous=120.0, entry=100.0, current=130.0) == 130.0


def test_fractional_s1_breach_closes_entire_position_without_integer_cast(monkeypatch, tmp_path):
    client = FakeClient([pos("FRAC", 3.75, 100.0, 74.0)])
    monkeypatch.setattr(soft, "assert_paper_env", lambda: None)
    monkeypatch.setattr(
        soft,
        "load_tracker",
        lambda: {"FRAC": {"system": "system1", "entry_date": "2026-09-01", "entry_price": 100.0}},
    )
    monkeypatch.setattr(soft, "load_symbol_system_map", lambda: {})
    monkeypatch.setattr(soft, "_order_system_index", lambda _client: {})
    monkeypatch.setattr(soft, "_bootstrap_high_from_cache", lambda *args, **kwargs: 100.0)
    monkeypatch.setattr(soft, "_write_status", lambda rows, stamp: tmp_path / "status.json")
...
def test_whole_share_s1_is_ignored_to_avoid_racing_native_trailing(monkeypatch, tmp_path):
    ...
    result = soft.run_soft_trailing(client=client, state_path=tmp_path / "trailing.json")
...
def test_s4_uses_20_percent_soft_trail(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        soft,
        "load_tracker",
        lambda: {"S4F": {"system": "system4", "entry_date": "2026-09-01", "entry_price": 100.0}},
    )
    monkeypatch.setattr(soft, "load_symbol_system_map", lambda: {})
    monkeypatch.setattr(soft, "_order_system_index", lambda _client: {})
    monkeypatch.setattr(soft, "_bootstrap_high_from_cache", lambda *args, **kwargs: 100.0)
...
def test_existing_monitor_cron_now_supports_step_and_overnight_session():
    ...
    assert not pred(datetime(2026, 9, 12, 22, 0))   # Saturday session is not a weekday start
    assert not pred(datetime(2026, 9, 11, 21, 30))


def test_legacy_blanket_cancel_recreate_task_is_retired():
    assert runner.TASKS["update_trailing_stops"] is runner.task_update_trailing_stops_retired
...
def test_paper_submitter_and_open_run_keep_whole_share_entry_policy():
    ...
    submitter = (root / "scripts" / "paper_trading_submit.py").read_text(encoding="utf-8")
