...
def main() -> int:
    ...
    final_csv = ROOT / "data_cache" / "signals" / (f"signals_final_{today_name}.csv")
    ...
    for sysname, info in sorted(summary.items()):
        ...
        if info["extra_in_final"]:
            ...
            print("Present in final but not in persisted " "(likely from another source): " + extra_txt)
        if info["diagnostics"]:
            try:
                ...
                if isinstance(d, dict):
                    ex_syms = d.get("exclude_symbols") or d.get("exclude_reasons_symbols")
    ...
    if final_only_systems:
        ...
        print("\nSystems present in final CSV but with no persisted per_system file: " + systems_txt)
