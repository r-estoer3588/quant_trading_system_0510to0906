...
出力:
  ...
  logs/perf/pipeline_bench_<YYYYMMDD_HHMMSS>.csv (サマリー: フェーズ名,時間,候補数,等)
...
def _time_phase(phase_name: str, phase_func, *args, **kwargs) -> tuple[Any, PhaseResult]:
    ...
    try:
        ...
        print(f"[bench] {phase_name}: {duration:.2f}s, {symbols_in}→{symbols_out} symbols, {candidates_count} candidates")
...
def main():
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as cf:
        ...
        writer.writerow([
            "phase", "duration_sec", "symbols_in", "symbols_out", "candidates_count",
            "cache_stats", "system_breakdown", "memory_peak_mb"
        ])
    ...
    print(f"\nPhase breakdown:")
    for phase in result.phases:
        print(f"  {phase.phase:<20} {phase.duration_sec:6.2f}s  ({phase.symbols_in}→{phase.symbols_out} symbols, {phase.candidates_count} candidates)")
