...
sample=paths[:12]
cols_interest=['Date','Close','Volume','DollarVolume50','HV50','AvgVolume50','ATR_Pct','ATR_Ratio']
report=[]
for p in sample:
    sym=p.stem
    try:
...
for _,_,s in report:
    nan_counts['total']+=1
    if s['dv_raw']=='NaN': nan_counts['dv']+=1
    if s['hv_raw']=='NaN': nan_counts['hv']+=1
    if s['av_raw']=='NaN': nan_counts['av']+=1
    if s['atr_raw']=='NaN': nan_counts['atr']+=1
