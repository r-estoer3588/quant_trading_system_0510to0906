...
def generate_candidates_system4(
    ...
    **_unused_kwargs: Any,
                        ...
                        for s_sym, s_df in prepared_dict.items():
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    s_rsi_f = float("nan")
                                samples.append((f"{s_sym}: date={s_dt.date()} setup={s_setup} rsi4={s_rsi_f:.4f}"))
                                taken += 1
                                if taken >= 2:
                                    break
                            except Exception:
                                continue
                        if samples:
                            log_callback(("System4: DEBUG latest_only 0 candidates. " + " | ".join(samples)))
                    except Exception:
                        pass
            ...
            for dt_raw, sub in df_all.groupby("date"):
                ...
                try:
                    if df_all is not None and not getattr(df_all, "empty", False):
                        ...
                        merged_norm = normalize_candidate_frame(df_all, system_name="system4")
                except Exception:
                    try:
                        merged_norm = df_all.copy()
                    except Exception:
    ...
    if log_callback:
        ...
        log_callback((f"System4: Generated {total_candidates} candidates " f"across {unique_dates} dates"))
        ...
        if candidates_df is not None and not getattr(candidates_df, "empty", False):
            try:
                from common.candidate_utils import (
                    normalize_candidate_frame as _norm_cf,
                )

                merged_norm2 = _norm_cf(candidates_df, system_name="system4")
            except Exception:
                try:
