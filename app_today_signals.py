...
if st.button("▶ 本日のシグナル実行", type="primary"):
    ...
    # 最近キャッシュの有無でフォールバック先を決定
    recent_dir = Path(settings.DATA_CACHE_RECENT_DIR)
    cache_dir = (
        recent_dir
        if recent_dir.exists() and any(recent_dir.glob("*.csv"))
        else Path(settings.DATA_CACHE_DIR)
    )
    ...
    def load_minimal_history(symbol: str, days: int) -> pd.DataFrame:
        ...
        try:
            data = pd.read_csv(cache_dir / f"{symbol}.csv")
        except Exception:
            try:
                data = pd.read_csv(Path(settings.DATA_CACHE_DIR) / f"{symbol}.csv")
            except Exception:
                return pd.DataFrame()
        return data.tail(max_days)
